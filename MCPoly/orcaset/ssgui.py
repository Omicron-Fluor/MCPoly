...
def ssgui():
    ...
    def curveshow(curve, curve2, file, loc):
...
#        
    ...
    def intro_status3d(button):
            ...
            with output2:
                if roomox.value == False and coreox.value == False:
                    XyzToInp(file.value+'_{0:.3f}'.format(strain.value),\
                             inpname = file.value+'_{0:.3f}'.format(strain.value+forcestep[key]),\
        ...
        while 1:
            ...
            if s == 4 or s == 41:
                ...
                with case:
                    ...
                    display(widgets.VBox([stop, widgets.HBox([curve,curve2])]))
                #strain.value = strain.value + forcestep[key]
        ...
        if aa.value != 404:
            with output:
                display(fromshow)
            with output2:
                display(widgets.HBox([curve, curve2]))
    ...
    curve = widgets.ToggleButton(description = 'Stress-Strain Curve',\
                                 layout = widgets.Layout(width='48%'),\
                                 style = dict(font_weight='bold', text_decoration='underline'))
    curve2 = widgets.ToggleButton(description = 'Distance-Force Curve',\
                                 layout = widgets.Layout(width='48%'),\
                                 style = dict(font_weight='bold', text_decoration='underline'))
    ...
    ssout = widgets.interactive_output(curveshow, {'curve': curve, 'curve2': curve2,\
                                                   'file': file, 'loc': loc})
